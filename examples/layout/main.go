...
func buildUI() *core.Element {
	return VStack().
		...
		Children(
			Box().
				...
				Children(
					HStack().
						...
						Children(
							Text("Layout Components").TextColor(theme.White).FontSize(18),
			...
			HStack().Gap("16").FlexWrap(props.WrapWrap).Children(
				Spacer(),
				buildCenterDemo(),
				buildFlexDemo(),
				buildSeparatorDemo(),
			...
			VStack().Gap("12").Children(
				Text("Container Demo").TextColor(theme.Gray600).FontSize(12),
				...
				Container().
					...
					Children(
						VStack().Gap("6").Children(
							Text("Container (max-width aware)").TextColor(theme.Gray800).FontSize(15),
							Text("This box acts as a container. On narrow screens, content adapts.").TextColor(theme.Gray500).FontSize(12),
						),
					),
			),
...
func buildCenterDemo() *core.Element {
	return Center().
		...
		BoxShadow(0, 1, 4, 0, rgba(0, 0, 0, 10)).
		Children(
			Text("Center").TextColor(theme.Gray800).FontSize(14),
		)
}
...
func buildFlexDemo() *core.Element {
	return Flex(props.FlexDirectionColumn).
		...
		BoxShadow(0, 1, 4, 0, rgba(0, 0, 0, 10)).
		Children(
			Text("Flex").TextColor(theme.Gray800).FontSize(14),
			HStack().Gap("6").Children(
...
func buildSeparatorDemo() *core.Element {
	return VStack().
		...
		BoxShadow(0, 1, 4, 0, rgba(0, 0, 0, 10)).
		Children(
			Text("Separator").TextColor(theme.Gray800).FontSize(14),
			buildSeparatorLine(),
			Text("Below line").TextColor(theme.Gray500).FontSize(12),
		)
}
...
func tag(label string, c color.NRGBA) *core.Element {
	return Box().
		...
		Children(Text(label).TextColor(theme.White).FontSize(11))
}
